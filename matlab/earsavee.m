...
filename='experiment_5_sample/pixelsavee.csv';
fnameString="test"; 
...
for x=1:endVal  
 ...
 if min(az)<=-0.005 || max(az)>=0.009
    ...
    if(appStart~=0 && appEnd~=0 && appStart<appEnd && appEnd-appStart>=2.0 && appEnd-appStart<=4.5)
        disp(appStart);
        disp(appEnd);
        disp("==========");
        
        if(appEnd-appStart)>=3.0
            largeCount=largeCount+1;
        elseif(appEnd-appStart)<=1.2
        ...
        if(savedStart<=238)
             class="angry";
        elseif(savedStart>238 && savedStart<=492)
            class="disgust";
        elseif(savedStart>492 && savedStart<=738)
            class="fear";
        elseif(savedStart>738 && savedStart<=984)
            class="happy";
        elseif(savedStart>984 && savedStart<=1455)
            class="neutral";
        elseif(savedStart>1455 && savedStart<=1744)
            class="sad";
        elseif(savedStart>1744)
            class="surprise";
        end
...
Fn=Fsp/2;
y_highpass=highpass(numPhone(:,4),1,Fsp);
