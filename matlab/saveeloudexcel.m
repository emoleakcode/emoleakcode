...
filename='experiment_5_sample/loudsaveangry.csv';
fnameString="test"; 
...
Fs = 1/mean(diff(num(:,1)));  
y_highpass=highpass(num(:,4),5,Fs);
num(:,4)=y_highpass;
 ...
 if min(az)<=-0.005 || max(az)>=0.009
    ...
    if(appStart~=0 && appEnd~=0 && appStart<appEnd && appEnd-appStart>=2.0 && appEnd-appStart<=4.5)
        ...
        appEnd=0;
         ...
         %my_xls(filename,savedStart,savedEnd,endVal);
    end
...
function my_xls(filename,start,funcend,endVal)
